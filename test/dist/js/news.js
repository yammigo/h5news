...
!function (window, $, undefined) {
    ...
    var template = {
        ...
        render: function () {
            var data = arguments[0], i = 0, str = "",renderType=arguments[1];
            ...
            //广告是否需要误点
            // if(&&(Math.random()<=probability)&&index!==0)


            var datalen=data.length;
            //广告列表单页长度超过40启用懒加载加载广告
            var async=datalen>15?true:false;
            ...
            if (datalen> 0) {
                for (i; i < datalen; i++) {
                    ...
                    if (item.category != "图片" && item.pics && item.pics.length < 3) {
             
                        str += this.styleimg1(item,i);
                    } else if (item.category != "图片" && item.pics && item.pics.length >= 3 && !item.type) {

                        str += this.styleimg3(item,i);
                    } else if (item.category == "图片") {

                        str += this.pricImg(item,i);
                    } else if (item.type = 'ad') {
                        if(adConfig.hasOwnProperty('isc') || hasctr.getP()){
                           
                        }else{
                            str += this.adStyle(item,i,async);
                        } 
                    }
    ...
    function initPage(posdata) {
        ...
        //上报页面接口刷新
            hasctr.setDown();
            var runderData = this.render.apply(this,[data,getType[0]]);
    ...
    function downRef(postdata) {
        ...
        template.getdata(clisturl, postdata, getType[1], function (data) {
            ...
            var runderData = this.render.apply(this, [data,getType[1]]);
    ...
    function upref(postdata) {
        template.getdata(clisturl, postdata, getType[2], function (data) {
            ...
            hasctr.setUp();
            //The function return dom and listdata
            var runderData = this.render.apply(this, [data,getType[2]]);
    ...
    function chechRender() {
        ...
        var runderData = template.render.apply(template, [JSON.parse(sessionStorage.getItem("chechData")).data,getType[0],true]);
        utils.clipImg(runderData.dom.find('img'));
        $(".ZZJK_L .news-list").append(runderData.dom);
        mescroll.lazyLoad(200);
        //解决懒加载bug
