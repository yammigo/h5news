...
!function (window, $, undefined) {
    ...
    var template = {
        ...
        dilution: function (adcount) {
            var cachead = JSON.parse(sessionStorage.getItem('dilution'));
            if(!cachead && adcount){
                sessionStorage.setItem('dilution',JSON.stringify(adcount))
            }
            var adcount = adcount || cachead;
            if (!!adcount){
                $("#track").empty();
                for (var indexad = 0,len = adcount.length;indexad < len;indexad++) {
                    
                    //    $("#track"+" .track"+index).append(crateAd.init(adcount[index],false));
                    (function (ad) {
                        var adid = "zzjk_" + Math.random().toString(36).slice(2);
                        ad.domid=adid;
                        $("#track").append('<div id="' + adid + '">'+crateAd.init(ad,false)+'</div>');
        ...
        readAd: function (isAD, data, adIndex) {
            ...
            var adcount = shuffle(adlist);
            var newadlist = adcount.splice(0,5);
            this.dilution(adcount.slice(0,5));
            isAD && (function (that, data, newadlist) {
                var i = 0;
                for (i; i < len; i++) {
                    if (newadlist[i]) {
                        data.splice((adIndex + 1) * i, 0, newadlist[i]);
                    }
                }
            }(this, data, newadlist))
