...
(function (name, definition) {
    ...
    var timer = setTimeout(function () {
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (dom.getAttribute(me.lazyTag) !== 'true' && me.isInSee(dom, me.optUp.lazyLoad.offset)) {
          ...
          if(dom.tagName==="IMG"||dom.tagName==="DIV"){
            var temp = new Image();
          ...
          if(dom.tagName==="SCRIPT"){
                ...
                ad.src=imgurl;
                ad.onload=function(){
                  ad.removeAttribute(me.optUp.lazyLoad.attr);
                  ad.removeAttribute(me.lazyTag);
                }
                dom.setAttribute(me.lazyTag, 'true');
                dom.parentNode.appendChild(ad);
  ...
  MeScroll.prototype.isInSee = function (dom, offset) {
    ...
    var dom=dom.tagName=="SCRIPT"?dom.parentNode:dom;
    var topDom =this.getOffsetTop(dom);// 元素顶部到容器顶部的距离
    // console.log(this.getOffsetTop(dom.parentNode));
